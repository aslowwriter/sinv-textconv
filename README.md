...
`sinv-textconv` is a cli program for use as a `git textconv` option to view diffs between Sphinx inventory files (typically `objects.inv`). `textconv` is a git configuration option to instruct git to use a certain program to convert binary files to plain text so that a diff can be displayed. 

If you keep `objects.inv` in your history, (or want to diff them yourself) this programme offers a fast and convenient way to do that. 

This project is directly modeled after [sphobjinv-textconv](https://sphobjinv.readthedocs.io/en/stable/cli/textconv.html)

## Features

`sinv-textconv` is kept intentionally minimal in functionality so it is as light weight as possible and is optimised for its intended use case instead for cli convenience.

Specifically this means that: 
1. it takes _exactly_ one argument, that being the path to an inventory file, and it will output the context of that file in plaintext over stdout
2. the contents of the file are NOT parsed, and instead the entire zlib content is dummped directly into stdout. This is because it's not uncommon for Sphinx to produce references that don't conform to the format and thus can't be parsed (like happens in the linux kernel docs). You might still want to see the diff for these, hence the decision to not parse the contents.

If you are looking for an application that is more optimised for direct user interaction please see [sinv](https://github.com/aslowwriter/sinv)
...
In case you want to do one-off diffs of files that aren't in the repository you can use process substitution: 
...
Comparing to [sphobjinv-textconv] when operating on the linux kernel documentation inverntory file, we perform significantly faster:
